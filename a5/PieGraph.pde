class PieGraph{
  ...
  boolean stripes;
  PieGraph(float posx1, float posy1, float w1, float h1, String[] keys1, float[] values1, boolean stripes1) {
    stripes = stripes1;
...
fill(255, 255, 255);    
  ...
  if (stripes == true) {
    ...
    while (diameter2 > 0) {
      ...
      arc(posx, posy, diameter2, diameter2, lastAngle, lastAngle+ radians(angles[i]));

      iter++;
      diameter2 = diameter2 - (diameter * (abs(radians(angles[i])) / (2*PI)));
    }
  }
